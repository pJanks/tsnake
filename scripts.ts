...
const defaultsTable: TableObject = {
...
const makeNetworkRequest = async (url: string, options?: Options): Promise<void | HiScores[]> => {
  try {
...
const padNumber = (numberToPad: number): string => String(numberToPad).padStart(2, '0');
...
const checkForTailCollision = (head: SnakePiece): boolean => {
...
const checkForPillCollision = (head: SnakePiece): boolean => {
  if ((xVelocity && head.x + 5 === pillXValue) && head.y + 5 === pillYValue || (yVelocity && head.y + 5 === pillYValue) && head.x + 5 === pillXValue) {
    ...
    const scoreDetails: TableObject = {
      intervalRunsIn: `${timeout} ms`,
      nextPillIsWorth: points,
      score,
      pillsEaten,
